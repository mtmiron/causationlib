...
static inline unsigned char get_faded_color(float fade_time, TortoiseTime &time_delta)
{
	...
	diff = round(time_delta.tv_sec * pow(10,9) + time_delta.tv_nsec) / (max / 255.0);
	return 255 - diff;
}
...
Mat NeuralNet::createActivityImage(int width, int height, TortoiseTime at_time, float fade_time, bool draw_weak_stimulation)
{
	Mat image(height, width, CV_8UC3, Vec3b(0,0,0));
	Vec3b pixel(0,0,0);
	Vec3b color(0,0,0);
